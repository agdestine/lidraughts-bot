...
class DXPEngine:
    ...
    def _start(self, board: draughts.Game, time: int) -> None:
        """Start the game."""
        print("--- HERE in START with time = "+ str(time))
        self._connect()
    ...
    def _recv_move(self) -> Optional[List[List[int]]]:
        ...
        print("--- HERE in _recv_move")
        while True:
            if not dxp.tReceiveHandler.isListening:
                print("---- ERROR OOPS : if not dxp.tReceiveHandler.isListening")
                break
            if self._last_move != dxp.last_move:
                self._last_move = dxp.last_move
                logger.debug(f'new last move: {self._last_move}')
                print("---- self._last_move : "+ self._last_move)
                return self._last_move
            #print ("self._last_move:"+str(self._last_move))
            #print ("dxp.last_move:"+str(dxp.last_move))
